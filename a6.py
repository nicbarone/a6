# a6.py
# Yuxin Xu (yx67), Nicolas Barone (njb227)
# 11/16/16
...
def is_point_list(thelist):
    ...
    for x in thelist:
        if is_point(x) == False:
           return False 
        if len(x) != len(thelist[0]):
           return False
        
    return True


def is_valid_indices(thelist,m):
    for x in thelist:
        if not x >= 0 and x <= m:
           return False 
    return True        
...
class Dataset(object):
    ...
    def __init__(self, dim, contents=None):
        ...
        """
        # IMPLEMENT ME
        assert is_point_list(contents) or contents == None
...
class Cluster(object):
    ...
    def distance(self, point):
        ...
        for x in range(self._dataset.getDimension()):
            sum = sum + math.pow((self.getCentroid()[x]-point[x]), 2)
    ...
    def updateCentroid(self):
        ...
        #new _centroid attribute
        new = []
        n = self.getContents()
        
        #If no points in the cluster, the centroid does not change.
        if n == []:
           return True 
        ...
        for x in range(self._dataset.getDimension()):
            ...
            for k in range(len(n)):
                sum = sum + n[k][x]
            m = sum/len(n)  #first/second... coordinate
            new.append(m)
        
        if numpy.allclose(new,self.getCentroid()):
           return True
        else:
           self._centroid = new
           return False
...
class ClusterGroup(object):
    ...
    def __init__(self, ds, k, seed_inds=None):
        ...
        assert seed_inds == None or (len(seed_inds) == k
                                    and is_valid_indices(seed_inds,ds.getSize()-1))
        
        self._dataset = ds
        self._clusters = []
        if seed_inds == None:
           x = random.sample(range(ds.getSize()),k)
           for y in x:
               self._clusters.append(Cluster(ds,ds.getPoint(y)))
        else:
           for z in seed_inds:
               self._clusters.append(Cluster(ds,ds.getPoint(z))) 
    ...
    def _nearest_cluster(self, point):
        ...
        assert is_point(point)
        assert len(point) == self._dataset.getDimension()
        x = self.getClusters()[0]
        for y in self.getClusters():
            dis =  y.distance(point)
            if dis < x.distance(point):
               x = y 
        return x
    ...
    def _partition(self):
        ...
        # IMPLEMENT ME
        for m in self._clusters:
            m.clear()
        for x in range(self._dataset.getSize()):    
            thecluster = self._nearest_cluster(self._dataset.getPoint(x))
            thecluster.addIndex(x)
